    ...
    // 将社交数据映射到价格时间点
     const kolMarkers: KolMarker[] = demoData.social
      ...
      .map((social) => {
        ...
        // 找到与社交数据对应的时间段的索引
        // const index = processedPrices.findIndex((p) =>
        //   granularity === "1H"
      ...
      const kolMarkers2 = kolMarkers.filter(v=>v.x>0).map((point, index, arr) => {
        // 找出同一天的数据点
        const sameDayPoints = arr.filter(p => p.x === point.x);
        
        if (sameDayPoints.length > 1) {
          // 为相同日期的点引入偏移量
          const offset = sameDayPoints.indexOf(point) * 0.1;
          return { ...point, x:point.x+offset }; // 或者其他格式
        }
        
        return point;
      })
    console.log(processedPrices.map((item) => ({
      ...item,
    ...
    <div className="flex flex-col gap-8 min-h-screen p-8 bg-black text-white">
      <section>
                ...
                {(["0-5k", "5k-10k", "10k-50k", "50k+"] as const).map(
                  (range) => (
                    <button
                      ...
                      className={`px-3 py-1 text-sm rounded ${followerFilter.includes(range)
              ...
              <div className="flex gap-2">
                {(["1H", "1D", "1W"] as const).map((gran) => (
                  <button
                    ...
                    className={`px-3 py-1 text-sm rounded ${granularity === gran
                        ? "bg-blue-500 text-white"
                        : "bg-gray-800 text-gray-300 hover:bg-gray-700"
                      }`}
                  >
        ...
        <div className="relative h-[600px] w-full">
          ...
          {chartData.kolMarkers2.map((marker, index) => {
                        ...
                        return (
                          <div className="flex flex-col gap-1">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={`font-bold ${isProfit ? "text-green-500" : "text-red-500"
                                  }`}
                              >
                                {isProfit ? "+" : ""}
